...
| 6 | Character Capability Manifest | Partial | Runtime derivation, strict validation, a V1 schema, and focused tests exist. The authenticated `GET /api/avatar/wizard/capabilities` query now exposes the exact verified active manifest, its digest, runtime admission identity, and active namespaced epoch without deriving or admitting anything on request. Wizard Joe's checked-in package still exposes six broad package strings, and the other JoeVille libraries remain source-motion candidates rather than admitted capability manifests. |
