...
- Python/Companion: `/Users/paul/Documents/WizardJoeAsci/worktrees/wizardjoe-character-director`
  on `codex/character-director` at code receipt commit `4a5af34bef9166e5ffe3fe9651aa7de50935bf9b`.
...
| 21 | Eye movement has been reviewed. | **Partial** | Transactional presentation and simulation-tick head-eye state are covered by focused race, rerender, retarget, reconnect, and actual-pixel tests. Two independent reviewers scored the captured gaze/head-eye material 2/4: gaze is readable, but changes are abrupt and the required visible eye-lead/head-follow/settle sequence is not proven. |
...
| 23 | Hand preparation, stroke, hold, and recovery have been reviewed. | **Partial** | The new independent animation review scores the observed cast 3/4, improved from the prior 2/4. Preparation, stroke, and hold are clearer, but the effect begins early and persists after the body settles. Three repeated casts, marker/anchor measurements, and V7 interrupt variants remain open. |
| 24 | Locomotion starts, stops, turns, and foot contact have been reviewed. | **Partial** | Clean candidate `4a5af34` now executes graph transitions on the 60 Hz authoritative timeline, waits for minimum holds/contact/markers, rebases ground-distance phase, follows run charge with authored recovery, settles a stopped walk to contact, and uses complete front-walk strides for horizontal stage travel. Two independent reviews reject release while recording material improvement: locomotion increased from 1/4 to 2/4 and interruption from 1/4 to 3/4, with the old reset displacement eliminated. A reversal back-flash, stop/profile pop, abrupt turns, and unmeasured zero-cell planted-foot drift remain open. |
...
| 32 | Expensive work does not block the render or UI loop. | **Implemented+verified** | `WizardFrameHub` prepares scores off-loop and renders outside the runtime lock; exact retained-replay serialization/hashing is excluded from per-frame diagnostics and remains available on explicit diagnostics/export paths. Transition lifecycle now advances only in authoritative simulation resolution; repeated render candidates cannot initiate or advance it, and stale candidates remain atomically rejectable. Focused purity/off-loop tests and the prior concurrency soak pass. |
...
| 36 | Automated tests pass. | **Implemented+verified** | Current Python discovery contains 501 passing tests and the production-scope validator scans 65 files with zero violations. New tests cover marker/contact gates, phase rebasing, stopped-contact settle, run recovery, render purity during pending transitions, horizontal gait, speech continuity, and stale TTS callbacks. The paired permission-authority and prior Companion/Prism gates remain recorded in `PRODUCTION_VERIFICATION.md`. |
...
2. **The replacement director review rejects release.** The clean-commit
   run under `evidence/character-director/authored-transition-2026-07-18-4a5af34/`
   retains 341 contiguous 24 FPS frames with zero detected loss and manifest
   SHA-256 `9b62cf3967d5faa8c04e744977ab38ab75874016fbdee78c134af6c331892522`.
   The two independent reviews record an observed-only score increase from
   50.0% to 65.9%, elimination of the old interruption displacement, and less
   static side skating. They also identify a reversal back-flash, stop/profile
   pop, abrupt turns, detached cast-effect timing, unmeasured planted-foot
   drift, and an incomplete V1-V10/browser/audio/reduced-motion evidence
   matrix. See `reviews/REVIEW_SYNTHESIS.md` in that evidence package.
