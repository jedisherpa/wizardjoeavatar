...
| 23 | Hand preparation, stroke, hold, and recovery have been reviewed. | **Implemented+verified** | V3 accepts three complete canonical casts. V7 evidence at `8217ccc2` dispatches interruption from accepted `cast_front@8` and `action_commit@10`, retains exact commit/effect/recoverable/settled events at authored frames 10/14/23/28, presents every recovery drawing 23-30, and starts only the identified replacement speech afterward. The 173-frame run has zero drops, clipping, root drift, or contact issues and retains normal- and quarter-speed review products. |
