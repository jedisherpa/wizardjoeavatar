...
- Python/Companion: `/Users/paul/Documents/WizardJoeAsci/worktrees/wizardjoe-character-director`
  on `codex/character-director` at code receipt commit `4903699c3f6a9a627fd9b52ee3e5058f5e9c2629`.
...
| 21 | Eye movement has been reviewed. | **Implemented-not-visually-verified** | Gaze targets reach authoritative state and pixel bounds are tested in `test_frame_source.py`, `test_game_control.py`, and `test_performance_application.py`. The strict real-runtime harness now captures contiguous gaze-left/right ranges and square-cell artifacts, but the required slow-motion, head-eye lead, and two-reviewer score remain open. |
| 22 | Blink timing has been reviewed. | **Implemented-not-visually-verified** | `BlinkScheduler` replaces the metronomic 4.2-second cycle with replay-stable 2.5-6.5-second open intervals and 100-200 ms closures; focused tests prove timing variety, reset reproducibility, aperture compatibility, and root/body isolation. A 60-second connected blink review and two-reviewer score remain open. |
| 23 | Hand preparation, stroke, hold, and recovery have been reviewed. | **Partial** | Score phases and clip commit/recovery markers exist and the strict harness records the full 24 FPS cast range with wire/frame hashes and contact sheets. The required quarter-speed hand/staff arc review, phase metrics, and two-reviewer score remain open. |
| 24 | Locomotion starts, stops, turns, and foot contact have been reviewed. | **Partial** | Graph entry now honors preserve/restart/nearest-contact phase policy, and focused tests cover deterministic contact matching. The strict harness records walk, reversal, stop, and front/back turns at 24 FPS, but planted-foot metrics and the required scored slow-motion review remain open. |
...
| 36 | Automated tests pass. | **Implemented+verified** | Current Python discovery contains 472 passing tests and the production-scope validator scans 64 files with zero violations. The paired permission-authority checkpoint passes 674 Prism core tests and 307 Prism CLI tests. Prior green gates remain: Companion frontend (27/27), Companion Rust (17/17), strict animation quality (32/32), Prism frontend (40/40), Prism build, and locked release build. Exact commands are recorded in `PRODUCTION_VERIFICATION.md`. |
