...
| 24 | Locomotion starts, stops, turns, and foot contact have been reviewed. | **Partial** | Candidate `b7b6101` keeps horizontal reversal in `walk_front`, adds authored stop clips, and records 302 grounded frames across 13 declared stances with maximum continuous-anchor drift `1.42e-14` cells and zero root residual. Both reviewers still score locomotion 2/4: `front_idle` reads as a reset inside the stride, reversal lacks anticipation, and stop ends with an unmotivated profile turn. The verifier still needs decoded-raster foot occupancy/span checks and support-alternation proof. |
