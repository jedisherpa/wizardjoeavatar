...
- Python/Companion: `/Users/paul/Documents/WizardJoeAsci/worktrees/wizardjoe-character-director`
  on `codex/character-director` at code receipt commit `e62ddaef8dd9124e4591529e4fb694d93b541c4d`.
...
| 23 | Hand preparation, stroke, hold, and recovery have been reviewed. | **Partial** | Both independent reviewers scored the observed cast 2/4. Its silhouette and prepare/stroke are readable, but recovery collapses to idle in about one frame and the effect tail is detached from the hand/staff action. Three repeated casts, marker/anchor measurements, and V7 interrupt variants remain open. |
| 24 | Locomotion starts, stops, turns, and foot contact have been reviewed. | **Partial** | Both independent reviewers scored observed locomotion 1/4. Whole-sprite frames remain coherent, but side travel visibly skates without convincing alternating contact, reversals use abrupt pose handoffs, and stops lack deceleration/follow-through. Contact-authored execution and measured planted-foot evidence are P0 work. |
...
| 36 | Automated tests pass. | **Implemented+verified** | Current Python discovery contains 495 passing tests and the production-scope validator scans 65 files with zero violations. The paired permission-authority checkpoint passes 674 Prism core tests and 307 Prism CLI tests. Prior green gates remain: Companion frontend (27/27), Companion Rust (17/17), strict animation quality (32/32), Prism frontend (40/40), Prism build, and locked release build. Exact commands are recorded in `PRODUCTION_VERIFICATION.md`. |
...
2. **The required director review rejects the current motion.** The clean-commit
   run under `evidence/character-director/transactional-head-eye-2026-07-18-e62ddae/`
   retains 342 contiguous 24 FPS frames with zero detected loss and two reviews
   bound to manifest SHA-256 `647e4e6743037198afa5dfba800304c30593b693227469b173534e5aa155aef2`.
   The reviews agree that raster presentation is coherent but locomotion skates,
   turns are abrupt pose handoffs, cast recovery is too short, and the complete
   V1-V10/browser/audio/reduced-motion evidence matrix remains absent.
